...
interface Particle {
  ...
  size:    number;
  emoji?:  string;
  bgColor?: string;     // if set, render a View square instead of emoji
  isRect?: boolean;
  x:       Animated.Value;
  y:       Animated.Value;
  opacity: Animated.Value;
  scale:   Animated.Value;
  rot:     Animated.Value;  // 0–1 maps to 0–360 deg
}
  ...
  switch (type) {
    ...
    case 'throw': {
      ...
      const impact = Array.from({ length: 7 }, (_, i) =>
        makeP(`d${i}`, CX + rnd(-30, 30), H * 0.75, {
          emoji: pick(dust),
          size:  rnd(10, 16),
        }),
      );
      return [
        makeP('main', W * 0.85, H * 0.55, { emoji: '🪨', size: 48 }),
        ...impact,
      ];
    }
    ...
    case 'hearts': {
      ...
      return hearts.map((emoji, i) =>
        makeP(`h${i}`, CX + rnd(-W * 0.35, W * 0.35), H * 0.78, {
          emoji,
          size: rnd(18, 36),
        }),
      );
    ...
    case 'fire': {
      ...
      return Array.from({ length: 16 }, (_, i) =>
        makeP(`f${i}`, CX + rnd(-W * 0.4, W * 0.4), H * 0.88, {
          emoji: pick(flames),
          size:  rnd(20, 38),
        }),
      );
...
function runAnimation(
  ...
  const dur = type === 'fire' || type === 'snow' || type === 'confetti' ? 2200 : 1400;
    ...
    switch (type) {
      case 'explode': {
        ...
        Animated.sequence([
          ...
          Animated.parallel([
            Animated.timing(p.x,       { toValue: tx,  duration: 700, easing: Easing.out(Easing.cubic), useNativeDriver: true }),
            Animated.timing(p.y,       { toValue: ty,  duration: 700, easing: Easing.out(Easing.cubic), useNativeDriver: true }),
      ...
      case 'throw': {
        if (p.emoji === '🪨') {
          // Arc from right to center-bottom
          Animated.sequence([
            Animated.parallel([
